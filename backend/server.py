...
@api_router.get("/companies/dashboard")
...
@api_router.post("/companies/compounds")
...
@api_router.get("/companies/compounds")
...
@api_router.post("/companies/invite")
...
@api_router.get("/companies/pricing/calculate")
...
@api_router.put("/companies/compounds/{compound_id}")
async def update_compound(
    compound_id: str,
...
@api_router.get("/companies/{company_id}/analytics")
...
@api_router.get("/account/selection")
...
@api_router.post("/account/select-type")
...
@api_router.post("/individual/register")
...
@api_router.get("/individual/dashboard")
...
@api_router.post("/individual/upgrade")
...
@api_router.get("/individual/pricing/calculate")
...
@api_router.get("/admin/super/accounts")
...
@api_router.post("/admin/super/access-account")
...
@api_router.get("/users/{user_id}/subscription", response_model=Optional[UserSubscription])
async def get_user_subscription(user_id: str, current_user: User = Depends(get_current_user)):
    """الحصول على اشتراك المستخدم"""
    try:
        # التحقق من الصلاحيات
...
async def get_monitoring_stats(current_user: User = Depends(get_current_user)):
    """Get comprehensive system monitoring statistics (Admin only)"""
    try:
...
@app.get("/api/monitoring/activities")
...
@app.get("/api/monitoring/charts")
async def get_monitoring_charts(
    days: int = 7,
    current_user: User = Depends(get_current_user)
):
    """Get chart data for monitoring dashboard (Admin only)"""
    try:
        if current_user.role != "admin":
            raise HTTPException(status_code=403, detail="Access denied. Admin only.")
...
@app.post("/api/monitoring/errors/{error_id}/resolve")
    ...
    try:
        ...
        if success:
            await ActivityLogger.log_activity(
                ...
                username=current_user.username,
                details=f"Marked error {error_id} as resolved"
            )
...
@api_router.post("/push/send-invoice-notification")
...
@api_router.post("/push/send-payment-reminder")
...
@api_router.post("/push/send-visitor-notification")
...
class ContractCreate(BaseModel):
    ...
    provider_phone: str = ""
    provider_email: str = ""
    category: str = "maintenance"  # maintenance, security, cleaning, utilities, other
    value: float = 0
    start_date: str
    end_date: str
    terms: str = ""
...
async def create_contract(data: ContractCreate, current_user: User = Depends(require_admin)):
    try:
        contract = {
            "id": str(uuid.uuid4()),
            "compound_id": current_user.compound_id,
...
async def get_contracts(status: Optional[str] = None, current_user: User = Depends(require_admin)):
    try:
        query = {"compound_id": current_user.compound_id}
        if status:
            query["status"] = status
...
async def update_contract(contract_id: str, data: ContractCreate, current_user: User = Depends(require_admin)):
    try:
...
async def delete_contract(contract_id: str, current_user: User = Depends(require_admin)):
    try:
...
# Contract expiry checker - runs with daily report
...
class ComplaintCreate(BaseModel):
    type: str = "complaint"  # complaint, suggestion, inquiry
    category: str = "general"  # general, maintenance, security, cleaning, noise, parking, financial, other
    title: str
    description: str
...
async def create_complaint(data: ComplaintCreate, current_user: User = Depends(get_current_user)):
    try:
        complaint = {
            ...
            "user_id": current_user.id,
            "user_name": current_user.full_name,
            "unit_number": data.unit_number or current_user.unit_number or "",
            "type": data.type,
            "category": data.category,
            "title": data.title,
            "description": data.description,
        ...
        await notify_compound_admins(
            ...
            content=f"{type_label} من {current_user.full_name}: {data.title}",
...
@api_router.put("/complaints/{complaint_id}/respond")
...
@api_router.get("/facilities/{facility_id}/availability")
...
@api_router.get("/facility-bookings")
...
@api_router.put("/facility-bookings/{booking_id}/status")
async def update_booking_status(
    booking_id: str,
    status: str,
...
@api_router.post("/facility-bookings/{booking_id}/cancel")
