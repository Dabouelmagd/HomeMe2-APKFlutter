...
async def get_owner_budget(
    ...
    all_payments = await db.payment_transactions.find(pay_query, {"_id": 0}).to_list(None)
    regular_sub_revenue = sum(p.get("amount", 0) for p in all_payments)

    # 2. Company subscription revenue
    company_subs = await db.company_subscriptions.find({}, {"_id": 0}).to_list(None)
    ...
    ads = await db.internal_ads.find({}, {"_id": 0}).to_list(None)
    ...
    revenues = await db.revenue.find(rev_query, {"_id": 0}).to_list(None)
    ...
    codes = await db.subscription_codes.find({}, {"_id": 0}).to_list(None)
