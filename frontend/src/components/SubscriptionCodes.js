...
const SubscriptionCodes = () => {
  ...
  const handleRenewCode = (code) => {
    setSelectedCodeForRenew(code);
    setShowRenewModal(true);
  };

  const confirmRenewCode = async () => {
    if (!selectedCodeForRenew) return;
    
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(
        `${API}/api/admin/subscription-codes/${selectedCodeForRenew._id}/renew`,
        {
          duration: selectedCodeForRenew.duration_months + '_months',
      ...
      if (response.data.success) {
        toast.success(t('code_renewed_successfully', 'Code renewed successfully!'));
        fetchCodes();
        setShowRenewModal(false);
        setSelectedCodeForRenew(null);
      } else {
        toast.error(response.data.message || t('error_renewing_code', 'Failed to renew code'));
      }
    } catch (error) {
      console.error('Error renewing code:', error);
      toast.error(t('error_renewing_code', 'Failed to renew code. Please try again.'));
    }
  };
