// HomeMe PWA Service Worker - DISABLED FOR DEBUGGING
const CACHE_VERSION = 'v4.0.0';

// Minimal service worker - no caching
self.addEventListener('install', (event) => {
  console.log('HomeMe: Service Worker installing (no cache)');
  self.skipWaiting();
});

self.addEventListener('activate', (event) => {
  console.log('HomeMe: Service Worker activating (clearing all caches)');
  event.waitUntil(
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames.map((cacheName) => {
          console.log('HomeMe: Removing cache:', cacheName);
...
// No caching - direct network requests only
